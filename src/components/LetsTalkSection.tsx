...
interface LetsTalkSectionProps {
  locale: string
  /**
   * `simple` — fade + pin like inner CMS pages.
   * `static` — no GSAP (About): content is visible, no pin.
   */
...
export default function LetsTalkSection({ locale, variant = 'default' }: LetsTalkSectionProps) {
  ...
  useEffect(() => {
    if (!wrapperRef.current || isStatic) return
    ...
    const ctx = gsap.context(() => {
      ...
      // Mobile/Tablet: Simple sequential scroll entrance (no pinning)
      mm.add('(max-width: 1023px)', () => {
        gsap.set(wrapperRef.current, {
          clipPath: 'inset(0% 0% 0% 0%)',
        })
        gsap.set([logoRef.current, headingRef.current, subtextRef.current, buttonRef.current], {
          opacity: 0,
          y: 30,
          scale: 1, // Reset desktop scale overrides
        })
        ...
        tl.to(logoRef.current, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' })
          .to(headingRef.current, { opacity: 1, y: 0, duration: 0.6, ease: 'power2.out' }, '-=0.4')
  ...
  const section = (
    <section
      ...
      style={isSimple || isStatic ? undefined : { clipPath: 'inset(50% 0% 50% 0%)' }}
    >
      ...
      >
        ...
        >
          <g clipPath="url(#clip_let_talk_logo)">
            {/* Main brand letters: rythro... (White) */}
            <path
            ...
            {/* .ai text inside the box (Red: #E52421) */}
            <path
          ...
          <defs>
            <clipPath id="clip_let_talk_logo">
      ...
      {/* Main Bold Heading (Your business works more efficiently...) */}
      <h2
        ref={headingRef}
        className="font-heading-4xl text-center text-white max-w-[1170px] leading-tight md:leading-[60px] text-[32px] md:text-[48px] select-text px-6 normal-case tracking-normal"
      >
        {t(translations.heading)}
      </h2>

      {/* Subheading: We implement AI agents and automation based on n8n */}
      <div
        ref={subtextRef}
        className="text-center text-white max-w-[1170px] select-text px-6"
      >
        <span className="font-light text-[20px] md:text-[30px] leading-[30px] md:leading-[48px] opacity-90">
          {t(translations.subheadingPrefix)}{' '}
        </span>
