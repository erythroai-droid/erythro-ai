...
export const legalPages: Record<LegalPageId, LegalPage> = {
  ...
  accessibility: {
    ...
    updatedAt: '2026-07-26',
    intro: {
      en: 'Erythro.ai is committed to making this website accessible to as many people as possible, in line with Israeli accessibility requirements and Israeli Standard 5568 (aligned with WCAG 2.0 Level AA principles). This statement describes our efforts and how to request assistance.',
      ru: 'Erythro.ai стремится сделать сайт доступным для как можно большего числа людей в соответствии с требованиями Израиля по доступности и стандартом Израиля 5568 (в духе WCAG 2.0 уровня AA). В заявлении описаны наши меры и способ запросить помощь.',
      he: 'Erythro.ai מחויבת להנגשת האתר לכמה שיותר אנשים, בהתאם לדרישות הנגישות בישראל ולתקן ישראלי ת״י 5568 (בהלימה לעקרונות WCAG 2.0 ברמת AA). הצהרה זו מתארת את מאמצינו וכיצד ניתן לבקש סיוע.',
    },
    sections: [
      {
        heading: {
      ...
      },
      {
        ...
        bullets: {
          en: [
            'Semantic structure and readable contrast on primary content.',
            'Keyboard-operable navigation for main interactive controls where feasible.',
            'An on-site accessibility panel (text size, contrast, motion preferences, and related aids).',
            'RTL support for Hebrew and appropriate language attributes.',
            'Descriptive labels and alternative text for key images where provided.',
          ],
          ru: [
            'Семантическую структуру и читаемый контраст основного контента.',
            'Управление с клавиатуры для основных интерактивных элементов, где это возможно.',
            'Панель доступности на сайте (размер текста, контраст, анимация и связанные настройки).',
            'Поддержку RTL для иврита и корректные языковые атрибуты.',
            'Понятные подписи и альтернативный текст для ключевых изображений, где он задан.',
          ],
          he: [
            'מבנה סמנטי וניגודיות קריאה בתוכן העיקרי.',
            'ניווט באמצעות מקלדת לרכיבים אינטראקטיביים עיקריים ככל הניתן.',
            'חלונית נגישות באתר (גודל טקסט, ניגודיות, העדפות תנועה ועזרים נוספים).',
            'תמיכה ב־RTL לעברית ומאפייני שפה מתאימים.',
            'תוויות ותיאורי alt לתמונות מפתח ככל שסופקו.',
          ],
        },
      },
      {
      ...
      },
      {
        ...
        paragraphs: {
          en: [
            'Some pages include rich motion, video backgrounds, or complex layouts that may not yet meet every success criterion for all users. CMS-driven media may occasionally lack complete alternative text until updated. We continue to improve accessibility as we ship new features. If you encounter a barrier, please tell us — we will try to provide the information in an accessible format.',
          ],
          ru: [
            'На отдельных страницах есть сложная анимация, фоновое видео или вёрстка, которые могут пока не полностью соответствовать всем критериям для всех пользователей. В материалах из CMS иногда может не хватать полного альтернативного текста до обновления. Мы продолжаем улучшать доступность. Если вы столлись с барьером — сообщите нам, мы постараемся предоставить информацию в доступном формате.',
          ],
          he: [
            'בחלק מהעמודים יש תנועה עשירה, רקעי וידאו או פריסות מורכבות שעדיין עשויים שלא לעמוד בכל קריטריוני ההצלחה לכל המשתמשים. מדיה ממערכת התוכן עלולה לעיתים לחסור בטקסט חלופי מלא עד לעדכון. אנו ממשיכים לשפר את הנגישות. אם נתקלתם במחסום — דווחו לנו ונשתדל לספק את המידע בפורמט נגיש.',
          ],
        },
      },
      {
      ...
      },
      {
        ...
        paragraphs: {
          en: [
            'If you believe accessibility obligations were not met, you may contact us first. You may also contact the relevant Israeli authorities responsible for accessibility enforcement according to applicable regulations.',
          ],
          ru: [
            'Если вы считаете, что обязательства по доступности не выполнены, сначала свяжитесь с нами. Также можно обратиться в уполномоченные израильские органы по надзору за доступностью согласно применимым нормам.',
          ],
          he: [
            'אם לדעתכם לא עמדנו בחובות הנגישות, ניתן לפנות אלינו תחילה. ניתן גם לפנות לגורמי האכיפה בישראל המוסמכים בנושא נגישות בהתאם לתקנות החלות.',
          ],
        },
      },
    ],
