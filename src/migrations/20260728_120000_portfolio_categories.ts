 ...
 *
 * Uses a session advisory lock because Next.js can open multiple Payload
 * connections in parallel during "Collecting page data", and concurrent
 * `CREATE TABLE ... serial` races on `pg_type` even with IF NOT EXISTS.
 */
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`SELECT pg_advisory_lock(728120000)`)

  try {
    // Prefer explicit sequences: concurrent SERIAL CREATE TABLE is not race-safe.
    await db.execute(sql`
      CREATE SEQUENCE IF NOT EXISTS "portfolio_categories_id_seq";
    `)
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS "portfolio_categories" (
        "id" integer PRIMARY KEY DEFAULT nextval('portfolio_categories_id_seq') NOT NULL,
    ...
    `)
    await db.execute(sql`
      ALTER SEQUENCE "portfolio_categories_id_seq" OWNED BY "portfolio_categories"."id";
    `)
    ...
    await db.execute(sql`
      CREATE SEQUENCE IF NOT EXISTS "portfolio_categories_locales_id_seq";
    `)
    await db.execute(sql`
      CREATE TABLE IF NOT EXISTS "portfolio_categories_locales" (
        "label" varchar,
        "id" integer PRIMARY KEY DEFAULT nextval('portfolio_categories_locales_id_seq') NOT NULL,
        "_locale" "_locales" NOT NULL,
        "_parent_id" integer NOT NULL
      );
    `)
    await db.execute(sql`
      ALTER SEQUENCE "portfolio_categories_locales_id_seq" OWNED BY "portfolio_categories_locales"."id";
    `)
    ...
    await db.execute(sql`SELECT pg_advisory_unlock(728120000)`)
  }
}
