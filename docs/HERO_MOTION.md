...
Цикл останавливается, когда hero-секция уходит из viewport (`IntersectionObserver` на обёртку `220vh` / sticky, не на `lg:fixed` слой). На десктопе opacity stage синхронизирован со scrub ScrollTrigger (как fade контента в `HeroAnimation`).
...
3. После settle обводка выезжает **слева**.
4. Hold → outline уходит вправо; FG — `kineticSlamOut` (разлёт + blur).
...
3. Небольшой gap между «AI» и плашкой (`0.18em`).
4. Outline проявляется (opacity/scale).
5. Выход: **весь ряд** (AI + красная плашка) гаснет одним `opacity` — без per-char skew/blur внутри плашки (иначе подпрыгивание из‑за `overflow` / `filter`).
...
4. Второе слово — каскад снизу вверх (stagger).
5. Outline выезжает **справа**.
...
3. Outline разрезан пополам по высоте: верх — справа, низ — слева; стыкуются в центре.
4. Выход: вся красная рамка уходит блоком (`exitBlockBlur` / мягкий уход без explode букв внутри overflow).
