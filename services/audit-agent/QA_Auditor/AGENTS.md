...
- **Обход**: до 5 коммерчески важных URL (контакты, услуги, меню, портфолио, цены) — статус, title, формы (включая модалку после клика по CTA), CTA, чат/мессенджер, soft-404.
- **Gemini-разбор** (Pro HTML, при `GEMINI_API_KEY`): вердикт по воронке и приоритетная доработка — блок после топ-3, до Lighthouse. Free/Diagnostic не вызывают Gemini. Модальная форма и WhatsApp/чат считаются валидным путём к заявке; в отчёте есть сноска «как считаем путь к заявке».
...
| `agent_forms` | Число форм, включая открытые по клику на CTA |
| `agent_word_count` | Объём текста (слова) |
| `agent_cta` | CTA (contact / заявка / WhatsApp / Telegram…) и мгновенный канал |
| `agent_soft404` | Soft-404 по title + текст |
| `agent_lang` | `html lang` |
| `agent_dir` | `dir` / direction |

Опционально при `GEMINI_API_KEY` и `AUDIT_TIER=PRO`: вердикт по воронке, пробелы, `priority_fix` — в Pro HTML после топ-3. Модальная форма и WhatsApp/чат — валидный путь к заявке; в блоке есть методологическая сноска.
