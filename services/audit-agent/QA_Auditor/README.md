...
7. **Агентный просмотр ключевых страниц**:
   - Сбор кандидатов из `sitemap.xml` и навигации, обход до N коммерчески важных URL.
   - Проверка статуса, форм (включая модалку после CTA), CTA, чат/WhatsApp и soft-404; Gemini-вердикт по воронке в Pro HTML (`GEMINI_API_KEY`, `AUDIT_TIER=PRO`).
